...
import {
  ...
  updateMemberRole, updateMemberPermissions, generateMemberPassword,
...
function MemberCard({ member, onRefresh, onRemove }: {
  ...
  async function handleGenPassword() {
    setGenLoading(true)
    const res = await generateMemberPassword(member.user_id)
    setGenLoading(false)
    if (res.error) { alert('Erro: ' + res.error); return }
    setGenPwd(res.password ?? null)
  }

  function handleCopy() {
    if (!genPwd) return
    navigator.clipboard.writeText(genPwd)
    setCopied(true)
    ...
    <div className={cn(
      ...
      {open && (
        <div className="px-4 pb-4 space-y-4 border-t border-gray-100/80 pt-4">
          ...
          {/* Acesso / senha temporária */}
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Acesso ao sistema</p>
            <div className="flex items-center gap-2 flex-wrap">
              <p className="text-sm text-gray-500 flex-1 min-w-0 truncate">
                {member.profile?.email ?? <span className="text-gray-300 italic">email não encontrado</span>}
              </p>
              <button onClick={handleGenPassword} disabled={genLoading}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary/10 text-primary text-sm font-medium hover:bg-primary/15 transition-colors disabled:opacity-60 shrink-0">
                {genLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <KeyRound className="w-3.5 h-3.5" />}
                Gerar nova senha
              </button>
            </div>
            {genPwd && (
              <div className="mt-2 flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-50 border border-amber-200">
                ...
                <span className="flex-1 font-mono text-sm font-semibold text-amber-800 select-all">{genPwd}</span>
                <button onClick={handleCopy}
                  className="flex items-center gap-1 px-2 py-1 rounded text-xs font-medium text-amber-700 hover:bg-amber-100 transition-colors">
            ...
            {genPwd && (
              <p className="text-[11px] text-amber-600 mt-1">⚠️ Copie agora — essa senha não será exibida novamente.</p>
            )}
          </div>
